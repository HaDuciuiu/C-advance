...
void exch(void *buf, int i, int j)
{
    ...
    // *((int *)buf + j) = tmp;
    int tmp;
    memcpy(&tmp, buf + i * sizeof(int), sizeof(int));
    memcpy(buf + i * sizeof(int), buf + j * sizeof(int), sizeof(int));
    memcpy(buf + j * sizeof(int), &tmp, sizeof(int));
}
...
void sort2way(int list[], int left, int right, int (*compare)(void *, void *))
{
    int pivot, i, j;
    if (left < right)
    {
        ...
        do
        {
            do
                i++;
            while (compare(&list[i], &pivot) == -1);
            do
                j--;
            while (compare(&list[j], &pivot) == 1);
            if (i < j)
                exch(list, i, j);
        } while (i < j);
        exch(list, left, j);
        sort2way(list, left, j - 1, compare);
        sort2way(list, j + 1, right, compare);
    }
}

void sort3way(int a[], int l, int r, int (*compare)(void *, void *))
{
    if (r <= l)
        return;
    int i = l - 1, j = r;
...
void SapXep(void *a, int l, int r, void (*sort)(int *, int, int, int (*)()))
{
    sort((int *)a, l, r, intCompare);
}
...
int main()
{
    ...
    SapXep(array1, 0, HUGE_NUMBER - 1, sort2way);
    end = clock();
    ...
    SapXep(array2, 0, HUGE_NUMBER - 1, sort3way);
    end = clock();
    cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
    printf("Run in %f seconds.\n", cpu_time_used);
    printf("\n----------------------------\n");
