...
int list(BTA *btact)
{
	...
	while (status == 0)
	{
		status = bnxtky(btact, key, &val);//next key
		if (status == 0)
		{
...
{
	btinit();// khoi tao gia tri NULL cho btfile
	btfile = btcrt("phonebook", 0, FALSE);
	int ch;
	char name[32];
	int sdt;
	int returnCode;// tra ve gia tri
	while (1)
	{
		printf(" --------------MENU--------------- \n"
		...
		case 1:
			...
			printf("SDT: ");
			scanf("%d%*c", &sdt);
			binsky(btfile, name, sdt);// chen vao cay
			printf("Insert Key Thanh Cong!\n");
			break;
		case 2:
			printf("Find Name: ");
			scanf("%[^\n]%*c", name);
			returnCode = bfndky(btfile, name, &sdt);// tim gia tri cua mot khoa
			if (returnCode == 0)
			{
		...
		case 4:
			printf("Delete Name: ");
			scanf("%[^\n]%*c", name);
			bdelky(btfile, name);// xoa du lieu
