...
HuffmanTreeArray makeHuffmanTreeArray(Coding htable[]) 
{
	...
	maxlen = 0;
	// duyet tim phan tu co so bit bieu dien lon nhat
	for (i=0; i<256; i++)
		if (htable[i].size > maxlen) maxlen = htable[i].size;
	size = 1;
	exp2 = 1;
	for (i=1; i<=maxlen; i++) {
		exp2 *= 2;	// so nut cua lop sau bang so lop cua tang truoc nhan voi 2
		size += exp2;//tong so nut hien tai - size => dem so nut tren cay
	}
	hArray.size = size;// kich thuoc cua mang bang so nut tren cay
	hArray.nodes = malloc(hArray.size*sizeof(int));
	for (i=0; i<hArray.size; i++) hArray.nodes[i] = -1;
	// duyet cay huffman
	for (i=0; i<256; i++) {
		setRoot(&cursor);// gan gia tri cho bien con tro cursor ve 0
		if (htable[i].size) {
			for (k=0; k<htable[i].size; k++) {
				if (htable[i].bits[k]==0) goLeft(&cursor);// neu bit thu k = 0 di xuong cay con trai
				else goRight(&cursor);// nguoc lai di den cay con phai
			}
